...
namespace Tensori.FPSHandsHorrorPack
{
    public class FPSItemSelector : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (SelectionOptions.Count > 0)
            {
                ...
                if (handsController != null)
                    handsController.SetHeldItem(defaultOption.ItemAsset, skipAnimation: true);
        ...
        private void Update()
        {
            if (handsController != null && handsController.CanChangeItem() == false)
                return;
            ...
            for (int i = 0; i < SelectionOptions.Count; i++)
            {
                ...
                {
                    if (handsController != null)
                        handsController.SetHeldItem(option.ItemAsset, skipAnimation: false);
